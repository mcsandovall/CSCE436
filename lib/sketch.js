    ...
    switch(key){
      ...
      case 'Enter':
        if (this.y >= 130 && this.y <= 150){
          if (this.x <= 550 && this.x >= 530) {
            pokemon1 = new Pokemon('pikachu', 'back');
            makePokemon( Math.ceil(random(0 , 3) ));
            game_state = 2;
          }

          if(this.x <= 570 && this.x >= 550){
            pokemon1 = new Pokemon('eevee', 'back');
            makePokemon( Math.ceil(random(0 , 3) ));
            game_state = 2;
          }

          if (this.x <= 620 && this.x >= 580){
            pokemon1 =  new Pokemon('psyduck', 'back');
            makePokemon( Math.ceil(random(0 , 3) ));
            game_state = 2;
          }
        }
...
function preload() {
  ...
  pokemon_moves = {
    ...
    'eevee' : {
      1 : {'name' : "Quick Attack", 'dmg' : 40},
      2 : {'name' : "Tackle", 'dmg' : 40},
      3 : {'name' : "Take Down", 'dmg' : 90},
      4 : {'name' : "Swift", 'dmg' : 60},
    },
    'psyduck' : {
      1 : {'name' : "Scratch", 'dmg' : 40},
      2 : {'name' : "Water Gun", 'dmg' : 40},
      3 : {'name' : "Aqua Tail", 'dmg' : 90},
      4 : {'name' : "Confusion", 'dmg' : 50},
    }
  }
...
function draw(){
  ...
  switch(game_state){
    case 0:
      startPage();
      if (keyIsPressed & key=="Enter"){ game_state+= 1; removeElements(); }
      break;
    case 1:
      labPage();
...
function startPage(){
  ...
  createDiv('Press Enter').addClass('main-action').center();
...
function makePokemon(index) {
  switch(index){
    case 1:
      pokemon2 = new Pokemon('pikachu', 'front');
      break
    case 2:
      pokemon2 = new Pokemon('eevee', 'front');
      break;
    case 3:
      pokemon2 = new Pokemon('psyduck', 'front');
      break;
  }
}